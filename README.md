# AppUpdater:强大且通用的AndroidApp版本升级库
## 写在前边
...
如果你只需要用到版本更新中app的下载、安装、进度回调、通知栏显示等核心功能，UI弹窗要自己开发，那么直接依赖core库即可。但是这样你需要对版本升级弹窗做更多功能和稳定性的调试，这可能需要花费较多的时间。
...
mAppUpdater.start();
```
只需要两行代码，即可完成下载安装。内部封装了一些列的默认配置
...
库core可以对下载进度进行监听，方便你在UI中做更新进度条、埋点等一系列操作。可扩展性非常强。
...
为保证最少三方库依赖度（内部只依赖了V7包），库core内默认使用了HttpsURLConnection进行网络请求下载。如果你想使用OkHttp等方式请求，请实现IHttpManager，覆盖download方法（下载功能）和cancel方法（取消下载）。将IHttpManager的实现类传入AppUpdater中即可。详细可以模仿或者参考内部已经写好的实现类HttpManager。
...
为了方便集成和快速使用，扩展库dialog内则封装了一个可使用的稳定的dialog。你集成以后可以不用做任何UI的工作，只需简单配置即可完成版本升级。
...
```
#### 步骤五：整合配置
将以上所有配置对象整合传入UpdateManager，通过start即可启动弹窗。
```
/**传入各种配置和参数，执行弹窗更新*/
...
这是因为你的下载链接URL为HTTP类型而非HTTPS的。建议在AndroidManifest.xml配置文件的<application>标签中直接插入
```
